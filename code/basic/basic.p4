...
control MyIngress(inout headers hdr,
    ...
    counter(8192, CounterType.packets) dropC;
    /* Stage 2 */
    ...
    register<bit<32>>(STAGE_BINS_SIZE) addr_table_1;
    register<bit<32>>(STAGE_BINS_SIZE) addr_table_2;
    register<bit<32>>(STAGE_BINS_SIZE) addr_table_3;
    register<bit<32>>(STAGE_BINS_SIZE) addr_table_4;
    
    
      action compute_hashes(){
       //Get register position
       hash(dedup_idx, HashAlgorithm.identity, (bit<32>)0, {hdr.ipv4.dstAddr, hdr.ipv4.srcAddr,hdr.tcp.checksum},(bit<16>)STAGE_BINS_SIZE);
    }
    ...
    apply {
        ...
        if (hdr.ipv4.isValid()) {
            ...
            addr =  hdr.ipv4.dstAddr;
            
             /* Dedup Stage 1 */
            dedup_table_1.read(key_1, dedup_idx);
            dedup_table_1.write(dedup_idx, key);
            addr_table_1.read(addr_1, dedup_idx);
            addr_table_1.write(dedup_idx, addr);
            if (key == key_1) {
            	if (addr == addr_1){
	    			dedup_drop = 1;
	    		}
	    ...
	    if(dedup_drop == 1) {
	    	drop();
	    	dropC.count((bit<32>) standard_metadata.ingress_port);
        } 
